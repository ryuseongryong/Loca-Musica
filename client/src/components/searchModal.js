...
function SearchModal({ isOpen, searchModalClose, setNowClickMusical }) {
    ...
    const searchKOPIS = () => {
        ...
        if (searchInput.stdate && searchInput.eddate && searchInput.cpage && searchInput.rows && (tempCpage > 0 && tempCpage < 1000) && (tempRows > 0 && tempRows < 1000)) {
                            ...
                            <div className='searchModal-section1'>
                                ...
                                <div className='searchModal-middle'>
                                    ...
                                    <div className='searchModal-middle-input'>
                                        <input type="date" placeholder='시작날짜 8자리(예시 : 20210811)' id='startdate' onChange={getStartDate} onClick={setMaxStartDate} />
                                        <input type="date" placeholder='종료날짜 8자리(예시 : 20210811)' id="endddate" onChange={getEndDate} onClick={setMaxEndDate} />
                                        <input type="number" placeholder='현재페이지(1~999)' id='currentpage' onChange={getCurrentPage} min='1' max='999' />
                                        <input type="number" placeholder='페이지당 목록 수(1~999)' id='rows' onChange={getRow} min='1' max='999' />
                                        <input type="text" placeholder='공연명(선택 옵션)' id='musicaltitle' onChange={getMusicalTitle} />
                                    </div>
                                    <div className='searchModal-middle-bottom'>
                                        <button className='searchModal-search-btn' onClick={searchKOPIS}>KOPIS 검색</button>
                                    </div>
                                </div>
                                ...
                                <ul className='searchModal-result-ul'>
                                    {kopisSearchList.length === 0 ?
                                        ...
                                        kopisSearchList.map((el) => (
                                            <li key={el.mt20id._text} className='searchModal-result-li'>
                                                <div className='searchModal-result-image'>
                                                    <img src={el.poster._text}
                                                        alt="searchModal-result-image" className='searchModal-result-post' onClick={getKopisMusical} />
                                                    <input type='hidden' value={`${el.prfnm._text}#${el.mt20id._text}#${el.prfstate._text}`} />
                                                </div>
                                                <div className='searchModal-result-input'>
                                                    <div className='searchModal-result-title'>
                                                        ...
                                                        <div className='searchModal-result-title-input'>{el.prfnm._text}</div>
                                                    </div>
                                                    <div className='searchModal-result-id'>
                                                        <div className='searchModal-result-id-info'>공연ID :</div>
                                                        <div className='searchModal-result-id-input'>{el.mt20id._text}</div>
                                                    </div>
